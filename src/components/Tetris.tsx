 ...
 */
function OriginkitBaseTetris(props: TetrisProps) {
  const {
  ...
  useEffect(() => {
    ...
    const gridRGB = parseColor(gridColor);
    ...
    function draw() {
      ...
      // Resting board: fill empty cells only if boardColor is non-transparent.
      if (boardRGB[3] > 0) {
        ctx.beginPath();
        for (let r = 0; r < rows; r++) {
      ...
      // Grid lines: stroke cell outlines if gridColor opacity > 0.
      if (gridRGB[3] > 0) {
        ctx.beginPath();
        for (let r = 0; r < rows; r++) {
          for (let c = 0; c < cols; c++) tilePath(c, r);
...
const __originkitPresetProps = {
  ...
  boardColor: "rgba(0, 0, 0, 0.04)",
  colors: ["#FF3800"],
  rounded: 0,
};

export default function Tetris(props: TetrisProps) {
  return (
    <OriginkitBaseTetris
