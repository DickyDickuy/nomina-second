...
export const fadeAnimation = () => {
    gsap.utils.toArray<HTMLElement>(".tp_fade_anim").forEach((item) => {
        const tp_fade_offset = item.dataset.fadeOffset
            ? parseInt(item.dataset.fadeOffset)
            : 40;
        const tp_duration_value = item.dataset.duration
            ? parseFloat(item.dataset.duration)
            : 0.75;
        const tp_fade_direction = item.dataset.fadeFrom || "bottom";
        const tp_onscroll_value = item.dataset.onScroll !== "0";
        const tp_delay_value = item.dataset.delay
            ? parseFloat(item.dataset.delay)
            : 0.15;
...
export const revalEffectAnimation = () => {
    ...
    mm.add({
        ...
        document.querySelectorAll(".tp_reveal_anim").forEach((areveal) => {
            ...
            const stagger = areveal.getAttribute("data-stagger")
                ? parseFloat(areveal.getAttribute("data-stagger") as string)
                : (isDesktop ? 0.02 : 0.08);
            const delay = areveal.getAttribute("data-delay") || 0.05;
            const split = new SplitText(areveal, {
                type: isDesktop ? "lines,words,chars" : "lines,words",
                linesClass: "tp-reveal-line",
            });
            const targets = isDesktop ? split.chars : split.words;
            const animConfig: gsap.TweenVars = {
                duration: isDesktop ? duration : 0.8,
                delay,
                ease: "power2.out",
                y: isDesktop ? 80 : 30,
                stagger,
...
export const charAnimation = () => {
    const charElements =
        document.querySelectorAll<HTMLElement>(".tp-char-animation");

    if (charElements.length > 0) {
        const animationItems =
            gsap.utils.toArray<HTMLElement>(".tp-char-animation");
...
export const servicePanelAnimation = () => {
    const panels = document.querySelectorAll(".tp-service-panel");
    if (!panels.length || !document.querySelector(".tp-service-pin")) return;
...
export const panelAnimation = (): void => {
    ...
    if (!panels.length || !document.querySelector(".tp-panel-pin-area")) return;
...
export const designChooseAnimation = () => {
    ...
    pw.add("(min-width: 1200px)", () => {
        document
            .querySelectorAll(".design-choose-item-wrap")
            .forEach((item) => {
                gsap.set(item.querySelector(".design-choose-item-1"), {
                    x: -400,
                    rotate: -40,
                });
                gsap.set(item.querySelector(".design-choose-item-2"), {
                    x: 400,
                    rotate: 40,
                });
                const tl = gsap.timeline({
                ...
                tl.to(item.querySelector(".design-choose-item-1"), {
                    ...
                    item.querySelector(".design-choose-item-2"),
                    { x: 0, rotate: 0 },
                    0,
                );
            });
...
export const textPerspectiveAnimation = () => {
    gsap.utils
        .toArray<HTMLElement>(".tp-text-perspective")
...
export const setupTextHoverEffect = (): void => {
    document
        .querySelectorAll<HTMLElement>(".codetext")
                    ...
                    { innerHTML: arr2.join("") },
                    {
                        ...
                        onUpdate: (): void => {
                            const progress: number = Math.floor(
                                tl.progress() * arr1.length,
                            );
                            if (step !== progress) {
                                step = progress;
                                arr1.forEach((_, i) => (arr2[i] = randChar()));

                                let start: string = arr1
                                    .slice(0, progress)
                                    .join("");
...
export const PortfolioSlicerAnimation = () => {
    if (window.innerWidth >= 1200) {
        const img1 = document.querySelector(".tp-portfolio-slicer-wrap .pro-img-1 img");
        const img2 = document.querySelector(".tp-portfolio-slicer-wrap .pro-img-2 img");
        if (!img1 && !img2) return;

        if (img1) gsap.set(img1, { x: 500 });
        if (img2) gsap.set(img2, { x: -500 });

        const portfolioWraps = gsap.utils.toArray<HTMLElement>(
...
export const scrollMovingText = () => {
    gsap.utils
        .toArray<HTMLElement>(".moving-text")
...
export const designProjectAnimation = () => {
    const items = document.querySelectorAll(".design-project-item");
    if (!items.length) return;

    const pw = gsap.matchMedia();
    pw.add("(min-width: 991px)", () => {
        const thumb1Global = document.querySelectorAll(".design-project-thumb.item-1");
        const thumb2Global = document.querySelectorAll(".design-project-thumb.item-2");
        if (thumb1Global.length) gsap.set(thumb1Global, { x: 400, rotate: 10 });
        if (thumb2Global.length) gsap.set(thumb2Global, { x: -400, rotate: -10 });
...
export const portfolioItemPinAnimation = () => {
    ...
    pi.add("(min-width: 1200px)", () => {
        document
            .querySelectorAll(".tp-pd-3-portfolio-item-wrap")
            .forEach((group) => {
...
// Footer vertical stretch (Studio Namma-style) for the big copyright text
// - scaleY only (no horizontal scaling)
// - anchored from top via transform-origin (stretches downward)
// - scrubbed between footer entering viewport and its bottom reaching viewport bottom
export const footerVerticalStretchAnim = () => {
    const mm = gsap.matchMedia();
    mm.add("(min-width: 768px)", () => {
        const footerTrigger = document.querySelector<HTMLElement>(
            ".tp-copyright-area.tp-copyright-style-6",
        );
        if (!footerTrigger) return;

        const stretchTarget = footerTrigger.querySelector<HTMLElement>(
            ".tp-footer-vertical-stretch",
        );
        if (!stretchTarget) return;

        // Avoid duplicate triggers on route transitions / re-mounts
        ScrollTrigger.getById("footerVerticalStretch")?.kill(true);
...
// team section – staggered entrance for the interactive team grid
export const teamSectionAnimation = () => {
    const items = gsap.utils.toArray<HTMLElement>(
        ".team-section-area .team-section-area",
    );
    // If no nested selector matches, target the grid rows via a more generic class
    const gridItems = items.length
        ? items
        : gsap.utils.toArray<HTMLElement>(".team-section-area");
